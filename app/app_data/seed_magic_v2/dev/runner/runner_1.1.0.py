...
async def run_seed(
        ...
        raw_data: List[dict],
        schema: Type[Schema],
        create_func: Callable[[AsyncSession, Schema], Awaitable[object]],
        ...
        if existing:
            if update_func:
                try:
                    ...
                    if updated:
                        updated_count += 1
                        logger.info(f"Updated fields for: {entity}")
                except (IntegrityError, HTTPException) as e:
                    logger.error(f"Error while updating: {e}")
            else:
                logger.info(f"Skipped (exists, no update func): {entity}")
        else:
            try:
                ...
                if result is not None:
                    ...
                    logger.info(f"Skipped (create skipped/exists): {entity}")
            except (IntegrityError, HTTPException) as e:
